...
import AppReducer from "./transactionReducer";
  ...
  const updateTransaction = async (transaction) => {
    try {
      ...
      const response = await axios
        .put(`/transactions/${transaction.id}`, transaction, config)
        .then(() => axios.get("/transactions"));

      dispatch({ type: "UPDATE_TRANSACTION", payload: response.data.data });
    } catch (error) {
      dispatch({
        type: "TRANSACTION_ERROR",
        payload: error.response.data.error,
